# LISA AI Hackathon: Machine Learning Methods for LISA

## Event Overview

This informal hackathon event focuses leveraging machine learning to tackle the data analysis challenges of the future LISA mission.

**Dates:** July 27–30, 2026  
**Location:** [ESTEC](https://maps.app.goo.gl/py45rHocKTnZSuCR6), Keplerlaan 1, 2201 AZ Noordwijk, Netherlands  

## Useful Information

- [Event Overview](#event-overview)
- [Hackathon Program](#hackathon-program)
- [Travelling Information](#travelling-information)
- [Accommodation](#accommodation)
- [Organizing Committee](#organizing-committee)
- [Keynote Speakers](#keynote-speakers)
- [Code of Conduct](#code-of-conduct)

---

## Hackathon Program

The Laser Interferometer Space Antenna (LISA) is one of the European Space Agency flagship mission and will be the first space-based gravitational wave observatory, opening a new observational window in the millihertz frequency regime. To fully exploit LISA's scientific potential, we must extract gravitational wave signals from a complex mixture of noise and overlapping sources. This hackathon aims to develop machine learning approaches that combine the speed of neural networks with the statistical rigor of Bayesian inference to address LISA data analysis challenges.

| Time | Monday, July 27 | Tuesday, July 28 | Wednesday, July 29 | Thursday, July 30 |
|------|----------------|------------------|--------------------|--------------------|
| **09:30–09:50** | | **Stephen Green** — Presentation: motivation, scientific framing, method advantages & disadvantages (20 min) | **Niklas Houba** — Presentation: motivation, scientific framing, method advantages & disadvantages (20 min) |  |
| **09:50–10:30** | | Tutorial Part 1 — Intro + pipeline; baseline solution delivered at end of session (40 min) | Tutorial Part 1 — Intro + pipeline; baseline solution delivered at end of session (40 min) | ESA LISA team presentation |
| **10:30–11:00** | | ☕ Break | ☕ Break | ESA LISA team presentation |
| **11:00–12:00** | | Tutorial Part 2 — Extensions + challenge + solution delivered at end of session (60 min) | Tutorial Part 2 — Extensions + challenge  + solution delivered at end of session (60 min) | ESTEC Visit |
| **12:00–12:30** | | ☕ Break | ☕ Break | ESTEC Visit |
| **12:30–13:00** | Registration | Jean-Baptiste Bayle: LISA Instrument and the Mojito Dataset | Pablo Gomez: Machine Learning in ESA missions | ESTEC Visit |
| **13:00–13:45** | Registration | 🍽 Lunch | 🍽 Lunch | 🍽 Lunch |
| **13:45–14:00** | Welcome + Practical Info | 🍽 Lunch | 🍽 Lunch | 🍽 Lunch |
| **14:00–14:20** | **Christoph Weniger** — Presentation: motivation, scientific framing, method advantages & disadvantages (20 min) | Talk session: <br>(1) Giada Badaracco: Posterior Homotopy: Stress-Testing Gravitational-Wave Inference | **Tom Dooney** — Presentation: signal separation motivation, scientific framing, method advantages & disadvantages (20 min) | **Maximilian Dax** — Presentation: motivation, scientific framing, method advantages & disadvantages (20 min) |
| **14:20–15:00** | Tutorial Part 1 — Intro + pipeline; baseline solution delivered at end of session (40 min) | (2) Uddipta Bhardwaj: Contrastive embedding for GW science <br> (3) Lucia Papalini: Transformers for GW | Tutorial Part 1 — Intro + pipeline; baseline solution delivered at end of session (40 min) | Tutorial Part 1 — Intro + pipeline; baseline solution delivered at end of session (40 min) |
| **15:00–15:30** | ☕ Break | ☕ Break | ☕ Break | ☕ Break |
| **15:30–16:30** | Tutorial Part 2 — Extensions + challenge + solution delivered at end of session (60 min) | (4) Louis Le Saulnier: SBI for fast parameter estimation of LISA MBHB <br> (5) Alice Spadaro: Accurate and efficient simulation-based inference for massive black-hole binaries with LISA <br> (6) Federico Fantoccoli: SBI for searching for EMRIs | Tutorial Part 2 — Extensions + challenge + solution delivered at end of session (60 min) | Tutorial Part 2 — Extensions + challenge  + solution delivered at end of session (60 min) |
| **16:30–17:00** | ☕ Break | ☕ Break | ☕ Break | ☕ Break |
| **17:00–17:30** | Round table of intros | Arek Skorski: Unlocking LISA Science with ESA Datalabs | Discussion | Discussion + closing remarks |
| **19:00** |  |  | Hackathon Dinner |  |
---

## Travelling Information

ESTEC is located in Noordwijk, on the Dutch coast, approximately 20–30 minutes from Leiden and 30–40 minutes from Amsterdam Schiphol Airport.

### Overall Useful Links and Tips

- [9292](https://9292.nl/en) is the best tool for door-to-door travel planning in the Netherlands. Install the app before arrival.  
- The [NS (Dutch Railways)](https://www.ns.nl/en) app is useful for train schedules and tickets.  
- Uber is available but less reliable than in other countries—do not rely on it.  
- Renting a bike is often the most efficient way to move locally (especially from nearby accommodation in Noordwijk or Leiden).

---

### By Plane

The nearest airport is [Amsterdam Schiphol Airport](https://www.schiphol.nl/en/), one of Europe’s main international hubs Alternative airport: [Rotterdam The Hague Airport](https://www.rotterdamthehagueairport.nl/en/)

---

### By Train

The closest major train station is **Leiden Centraal**.

Leiden is well connected to:
- Amsterdam
- The Hague
- Rotterdam
- Utrecht

From Leiden Centraal:
- Bus to Noordwijk / ESTEC (~20–30 minutes)  
- Taxi (~20 minutes)

For international routes, see:  
- [NS International](https://www.nsinternational.com/en)

---

## Accommodation

Participants are responsible for booking their own accommodation.

ESTEC is located in Noordwijk. The most convenient locations to stay are:
- **Noordwijk / Katwijk (closest, quiet, seaside)**
- **Leiden (best balance of accessibility and city life)**
- **The Hague (larger city, slightly longer commute)**

### Seaside (Katwijk aan Zee / Noordwijk aan Zee)
- [Boutique Hotel Noordeinde](https://www.booking.com/hotel/nl/boutique-noordeinde.en-gb.html)
- [Hotel Royal Noordwijk](https://www.booking.com/hotel/nl/royal-noordwijk.en-gb.html)
- [Two Brothers Hotel Astoria](https://www.booking.com/hotel/nl/astoria.en-gb.html)
- [Hotel Zee and Duin Katwijk](https://www.booking.com/hotel/nl/zee-en-duin.en-gb.html)
- [Hotel Noordzee](https://www.booking.com/hotel/nl/noordzee.en-gb.html)
- [Hotel Steeds aan Zee](https://www.booking.com/hotel/nl/b-b-aan-zee.en-gb.html)

### Leiden
- [Hotel Vic](https://www.hotelvic.nl/en)
- [Ibis Leiden Centre](https://all.accor.com/ssr/app/ibis/rates/8087/index.nl.shtml)
- [Golden Tulip & Tulip Inn](https://goldengreenhotels.nl)
- [Fletcher Wellness](https://www.wellnesshotelleiden.nl/nl/)
- [Intercity Hotel](https://hrewards.com/nl/intercityhotel-leiden)

### Nearby Leiden
- [Bastion Hotel Oegstgeest](https://www.booking.com/Share-T0CUUt)
- [Bastion Hotel Leiden Voorschoten](https://www.booking.com/hotel/nl/bastionleiden.en-gb.html)

### The Hague
- [NH Den Haag](https://www.booking.com/Share-VSn19B)
- [Babylon Hotel](https://www.booking.com/Share-3okVNF)
- [Mercure Hotel Den Haag Central](https://www.booking.com/Share-ySh4GJ)
- [Holiday Inn Express](https://www.booking.com/Share-7YQu0j)
- [Novotel Den Haag City Center](https://www.booking.com/Share-1ipoKT)
- [Ibis Styles Den Haag](https://www.booking.com/Share-1ipoKT)
- [Moxy The Hague](https://www.booking.com/hotel/nl/moxy-the-hague.en-gb.html)
- [Fletcher Hotel Leidschendam – Den Haag](https://www.booking.com/hotel/nl/mercure-den-haag-leidschendam.en-gb.html)

**Note:** ESA does not provide hotel booking services and has not authorized third parties to do so.
---

## Restaurants

- [Paco Ciao](https://maps.app.goo.gl/PnKFp39qbHU3q31J7) 
- [Hotspot](https://maps.app.goo.gl/eHx3RJSF5bFzTdST9)
- [Surakarta](https://maps.app.goo.gl/PPu2U1LvU9hxriYd9)
- [Sumatra House](https://maps.app.goo.gl/oHC6ojJgWk8cWJBg6)
- [Tabú](https://maps.app.goo.gl/hnhUfDvcJkZ7bNq86)
- [Mizūmi Ramen](https://maps.app.goo.gl/aR2CFvVftFCpxfqs8)
- [Ramen55](https://maps.app.goo.gl/K8MyNq2Do7p462Uz5)
- [Siri Thai](https://maps.app.goo.gl/qRj7r9nYaT3v5coXA)

---

## Organizing Committee

Lorenzo Speri

---

## Code of Conduct

We are committed to providing a welcoming, inclusive, and respectful environment for all participants.

All attendees are expected to:
- Treat colleagues with professionalism and respect
- Foster an inclusive atmosphere
- Respect diverse perspectives
- Report concerns to the organizing committee

Harassment or discrimination of any kind will not be tolerated.
